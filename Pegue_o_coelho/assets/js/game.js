var snake, coelho, squareSize, score, speed,
    updateDelay, direction, new_direction,
    addNew, cursors, scoreTextValue, speedTextValue, textStyle_Key, textStyle_Value;
var Game = {
    preload : function() {
        game.load.image('snake', './assets/images/cabeca.png');
    ...
    create : function() {
        ...
        //snake[0] = game.add.sprite(150+i*squareSize, 150, 'snake');        
        for(var i = 0; i < 10; i++){            
            snake[i] = game.add.sprite(150+i*squareSize, 150, 'corpo');      // Os parâmetros são (coordenada X, coordenada Y, imagem)
        }
    ...
    update: function() { 
        ...
        if (updateDelay % (10 - speed) == 0) {
            ...
            if(addNew){
                snake.unshift(game.add.sprite(oldLastCellx, oldLastCelly, 'corpo'));
                addNew = false;
            }
    ...
    coelhoCollision: function() {
        ...
        for(var i = 0; i < snake.length; i++){
            if(snake[i].x == coelho.x && snake[i].y == coelho.y){                
                addNew = true;
                coelho.destroy();
                this.generateCoelho();
                score++;
